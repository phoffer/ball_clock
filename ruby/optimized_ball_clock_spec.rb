...
RSpec.describe OptimizedClock do
  describe 'default parameters' do
    
    let(:clock_str) { OptimizedClock.new(count: 30, time: '5:32') }
    let(:clock_int) { OptimizedClock.new(count: 30, time: 332) }
    it 'should reject count outside 27..127' do
      expect{ OptimizedClock.new(count: 0) }.to   raise_error(BallCountError)
      expect{ OptimizedClock.new(count: 26) }.to  raise_error(BallCountError)
      expect{ OptimizedClock.new(count: 128) }.to raise_error(BallCountError)
    end
  ...
  describe 'ball counts at known time' do
    let(:clock) { OptimizedClock.new(count: 30, time: '5:32') }
  ...
  describe 'ball queues at known time' do
    let(:clock) { OptimizedClock.new(count: 30, time: 325) }
  ...
  describe 'it should circulate correctly' do
    let(:clock) { OptimizedClock.new(count: 30) }
